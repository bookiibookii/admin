...
| 항목 | 내용 |
|---|---|
| 메서드/경로 | `GET /api/admin/faq` (목록), `GET /api/admin/faq/{faqId}` (상세), `POST /api/admin/faq` (등록), `PATCH /api/admin/faq/{faqId}` (수정), `DELETE /api/admin/faq/{faqId}` (삭제) |
| 추가로 필요 | `GET /api/faq` — 앱(공개)에서 FAQ 목록을 노출하기 위한 조회 API |
| 필요 이유 | 앱 내 "자주 묻는 질문" 화면(질문/답변 아코디언 리스트)을 어드민에서 등록·수정·삭제할 수 있어야 함. 어드민 웹 페이지(`/faqs`)는 준비 완료, 백엔드 API 전체가 없는 상태. |
| 필요 요청 필드 | `question`(string), `answer`(string) |
| 필요 응답 필드 | `id`, `question`, `answer`, `createdAt`, `updatedAt` (정렬 순서가 필요하면 `displayOrder` 추가 고려) |
...
| FAQ 목록/등록/수정/삭제 | ❌ 미연동 | API 전체 없음. UI 페이지(`/faqs`) 준비 완료, 연동 대기 중 |
